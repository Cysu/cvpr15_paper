...
%%%%%%%%% ABSTRACT
\begin{abstract}
Large-scale strongly supervised datasets are crucial to train deep neural networks for various computer vision problems. Obtaining such well labeled data is usually expensive, but collecting web data with noisy labels is much more effortless. In this paper we model label noise with a graphical model and further integrate it into a deep learning framework. Experiments are conducted on a collected large-scale web dataset. The results show that noisy labeled data benefit the training of deep neural networks, and exploiting our label noise model can further improve the performance.
\end{abstract}
